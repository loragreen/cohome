    ...
    if ($slideshows.length) {
        $slideshows.each(function() {
            ...
            if ($slides.length && $slideWraper.length === 1) {
                ...
                var $arrows = $slideshows.children('.fr-widget.fr-img, .fr-widget.fr-svg'),
                    $arrowLeft = $arrows.eq(0),
                    $arrorRigt = $arrows.eq(1);

                if ($arrowLeft.length) {
                    $arrowLeft.on('click', function(){swiper.swipePrev();});
                }
                if ($arrorRigt.length) {
                    $arrorRigt.on('click', function(){swiper.swipeNext();});
                }
            }
        });
    }
